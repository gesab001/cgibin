...
command = "ffplay -vf subtitles=bible-subtitles.ass"+bible_style + ",subtitles="+movie_assfile+movie_style + " -i ../html/videos/"+movietitle+".mp4 -af \""
...
folder = "../html/video.onecloudapps.net/"
f = open(folder+movie_subtitle_file, "w")
f.write(subtitle_string)
#print(subtitle_string)
f.close()
convert_movie_assfile =  "ffmpeg -y -i " + folder+movie_subtitle_file + " "  + folder + movie_assfile
#print("convert_movie_assfile: " + convert_movie_assfile)
subprocess.call(convert_movie_assfile, shell=True)

f = open(folder+movie_subtitle_file)
text = f.readlines()
f.close()
vtt = "WEBVTT\n\n"
for line in text:
...
# convertoass =  "ffmpeg -y -i bible-subtitles.srt " + assfile
